...
- **Galgame 生图提示词与立绘承载**：伴侣头像和卡片立绘仍有内置 SVG 降级图，同时支持在伴侣数据里填写 `avatar` / `standee` 图片地址来替换为真实二次元立绘；12 位官方伴侣都整理了专属生图提示词。
...
{
  id: 'xinglan',
  avatar: './assets/xinglan-avatar.webp',
  standee: './assets/xinglan-standee.webp'
}
```

页面会优先使用 `avatar` / `standee` 图片；未填写时自动降级到内置 SVG，方便先用纯前端原型，后续再批量替换真实二次元立绘。
