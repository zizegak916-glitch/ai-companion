...
- **Galgame 生图提示词与立绘承载**：不再提交二进制图片文件：12 位官方伴侣会在浏览器运行时用 Canvas 生成 PNG data URL 二次元人物图，并通过 `<img>` 渲染为卡片立绘/头像；自定义伴侣仍保留 SVG 降级图，也可外接 `avatar` / `standee` 图片地址。
...
官方伴侣默认不写入二进制路径，而是由 `companionRaster()` 在浏览器里即时生成 PNG data URL；如果你后续有外部图床或 CDN 图片，也可以手动填写 `avatar` / `standee` 覆盖运行时生成图。
